...
def PreProcess(dirname, filename, var_amount=3):
    ...
    for imgfile in files:
        ...
        #img = load_img(imgfile, grayscale=True, color_mode='grayscale' target_size=(hw["height"], hw["width"]))    # 画像ファイルの読み込み
...
def Learning(tsnum=30, nb_epoch=50, batch_size=8, learn_schedule=0.9):
    ...
    mcp = ModelCheckpoint(filepath='best2.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
...
def TestProcess(imgname):
    ...
    model.load_weights("last2.hdf5")  # best.hdf5 で損失最小のパラメータを使用
    ...
    #shutil.copy(imgname, 'C:/Users/r201703362ko/zemi/dataset/suzu/suzu_'+textlist[np.argmax(pred)].replace(",", ""))
    #shutil.copy(imgname, 'C:/Users/r201703362ko/zemi/dataset/koba/koba_'+textlist[np.argmax(pred)].replace(",", ""))
    #shutil.copy(imgname, 'C:/Users/r201703362ko/zemi/dataset/kusa/kusa_'+textlist[np.argmax(pred)].replace(",", ""))
    #shutil.copy(imgname, 'C:/Users/r201703362ko/zemi/dataset/kuri_'+textlist[np.argmax(pred)].replace(",", ""))
...
dirname = "C:/Users/r201703362ko/zemi/dataset/kuri_close_open_test"#input("フォルダ名：")
#dirname = "C:/Users/r201703362ko/zemi/dataset/open_test"#input("フォルダ名：")
#dirname = "./dataset/close_test"#input("フォルダ名：")
files = glob.glob(dirname + "/*.jpg")
cn1 = 0; cn2 = 0;
